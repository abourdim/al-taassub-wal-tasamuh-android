#!/usr/bin/env bash
# ============================================================================
#  Al Taassub wal Tasamuh Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/17-al-taassub-wal-tasamuh}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.altaassubwaltasamuh}"
AVD_NAME="${AVD_NAME:-al_taassub_wal_tasamuh_test}"
...
cmd_test() {
  ...
  local out="/tmp/al-taassub-wal-tasamuh-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Al Taassub wal Tasamuh Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
