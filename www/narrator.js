/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Al-Taassub wal-Tasamuh
...
(function() {
  ...
  const STATE = {
    ...
    cardIndex: 0,
    cards: [],
    tabOrder: ['about','cards','tolerance'],
    tabIndex: 0,
  ...
  function cleanText(text) {
    ...
    if (l === 'fr') {
      clean = clean
        ...
        .replace(/Maa Allah/gi, 'Maa Allah')
    ...
    if (l === 'en') {
      clean = clean
        ...
        .replace(/Maa Allah/gi, 'Maa Allaah')
  ...
  function readDuoTranslation(card, onEnd) {
    ...
    const pMatch = principleId.match(/card-(\d+)/);
    if (pMatch && typeof CARDS !== 'undefined') {
      const p = CARDS[parseInt(pMatch[1]) - 1];
      if (p && p[duoLang]) {
    ...
    // Try anxiety data
    if (!duoText && card.type === 'anxiety' && typeof WISDOM_DATA !== 'undefined') {
      const anxietyCards = Array.from(card.el.parentNode.querySelectorAll('.anxiety-card'));
      const idx = anxietyCards.indexOf(card.el);
      if (idx >= 0 && WISDOM_DATA[idx] && WISDOM_DATA[idx][duoLang]) {
        const a = WISDOM_DATA[idx][duoLang];
  ...
  function onSpeedChange(val) {
    ...
    localStorage.setItem('tt-narrator-speed', STATE.speed);
  }
  ...
  function onPitchChange(val) {
    ...
    localStorage.setItem('tt-narrator-pitch', STATE.pitch);
  }
  ...
  function onKaraokeToggle(checked) {
    ...
    localStorage.setItem('tt-narrator-karaoke', checked);
  }

  function onAutoScrollToggle(checked) {
    STATE.autoScroll = checked;
    localStorage.setItem('tt-narrator-autoscroll', checked);
  }

  function onDuoToggle(checked) {
    STATE.duoReading = checked;
    localStorage.setItem('tt-narrator-duo', checked);
  }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('tt-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('tt-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('tt-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('tt-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('tt-narrator-duo') === 'true';
  }
